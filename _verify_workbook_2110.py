...
wb_path = Path(r"C:\Users\ZackerySimino\OneDrive - LightRiver Technologies Inc\Desktop\Inventory and Packing slips\BPA\TO3\Master_BPA_Task_Order_3_Raw_Report_2026-05-10_21-10.xlsx")
...
for sname in wb.sheetnames:
    ...
    for r_idx, row in enumerate(ws2.iter_rows(values_only=True), start=1):
        if any(v is not None and str(v).strip() == "Not Found" for v in row):
            not_found_cells += 1
            if len(not_found_rows) < 5:
                not_found_rows.append((sname, r_idx, row))

print(f"Literal 'Not Found' row count (rows containing at least one Not Found): {not_found_cells}")
if not_found_rows:
    print("Sample rows with Not Found:")
    for sname, r_idx, row in not_found_rows:
        print(f"  {sname} row {r_idx}: {row}")
