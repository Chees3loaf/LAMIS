"""
Toolkit-independent BOM comparison and workbook parsing engine.
    ...
    def _detect_detail_tab_layout(ws: Any) -> Optional[Tuple[int, Optional[int], Optional[int], int]]:
        ...
        for r in range(1, max_r + 1):
            ...
            for c in range(1, max_c + 1):
                ...
                if not s:
                    continue
                if pn_col is None and s in BomCompareEngine._DETAIL_PN_HEADERS:
                    pn_col = c
                elif desc_col is None and s in BomCompareEngine._DETAIL_DESC_HEADERS:
                    desc_col = c
                elif qty_col is None and s in BomCompareEngine._DETAIL_QTY_HEADERS:
                    qty_col = c
    ...
    def _write_site_allocation_sheet(
        ...
        * Both blank → site has neither ordered nor delivered units of
        ...
        for site in live_site_order:
            k = BomCompareEngine._site_key(site)
            if not k:
                continue
            live_sites_by_key.setdefault(k, []).append(site)
        sales_keys: set = {
            BomCompareEngine._site_key(s) for s in sal_site_order
        }
        # Live-only sites = Live tabs whose key never appears in Sales.
        live_only_sites = [
            s for s in live_site_order
            if BomCompareEngine._site_key(s) not in sales_keys
        ]
        ...
        for i, (pn, desc, _ordered, _live, _delta) in enumerate(
            ...
            for j, (_label, kind, src) in enumerate(site_cols):
                ...
                if kind == "sales":
                    ...
                    key = BomCompareEngine._site_key(src)
                    have_qty = sum(
