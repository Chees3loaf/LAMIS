...
- **check_updates()** checks for program updates
- **start_gui()** creates:
  - `CommandTracker`: Tracks which commands have executed per IP/connection type
  - `DatabaseCache`: Caches part descriptions from SQLite DB  
  - `InventoryGUI`: Main GUI window
...
run_inventory_worker(context)
  ...
  │       │   ├─ Try SSH first: SSH(admin/admin)
  │       │   │   └─ Run: "show system information"
  │       │   ├─ If SSH fails, try Telnet: Telnet(admin/admin)
  │       │   │   ├─ Run: "show general system-identification"
  │       │   │   └─ Run: "show general name"
  │       │   └─ parse_device_info(output) → (device_type, device_name)
  │       │       └─ Returns: "7250 IXR-R6", "7705 SAR-8", "1830", "6500", etc.
  │       │
  │       ├─ select_script(device_type, ip, stop_callback=should_stop)
  │       │   └─ Match device_type → import appropriate script:
  │       │       ├─ "7250 ixr-r6" → Nokia_IXR
  ...
  │           └─ Log completion
  │
  └─ Signal inventory_complete event to main thread
```
...
- When `("inventory_complete", True)` is received:
  - Updates status to "Exporting..."
  - Calls `start_export_worker(None)` → spawn Excel export thread
...
run_export_worker(context, _)
  └─ Call build_report_workbook(
       outputs=self.outputs,      # Dict of IP → inventory data
...
### Step 5c: build_report_workbook(outputs, output_file, ..., append_mode)
```
...
- **Main Thread**: GUI event loop, polling queue every 100ms
- **Inventory Worker Thread**: Ping → Identify → Execute commands
- **Export Worker Thread**: Excel workbook generation
- **Device Scripts**: Run via subprocess or direct import, support `stop_callback()`

### Data Structures
- **self.outputs{}**: Dict mapping IP → DataFrame with inventory data
  - Keys: IP addresses (strings)
  - Values: DataFrames with columns like "System Name", "Part Number", "Serial Number", etc.

- **self.run_queue**: Queue for inter-thread communication
  - Event types: "log", "inventory_complete", "export_complete", "error", "aborted"

- **summary_index{}**: Dict mapping IP → (device_name, sheet_title)
  - Used to track existing devices in append mode

### External Dependencies
- **script_interface.DeviceIdentifier**: Probes devices via SSH/Telnet
- **script_interface.ScriptSelector**: Imports and instantiates device-specific scripts
- **script_interface.is_reachable()**: Pings IP addresses
- **openpyxl**: Excel workbook manipulation
- **pandas**: DataFrame operations
- **sqlite3**: Part description lookups

### Device Scripts (scripts/*.py)
Each device script has:
- `get_commands()`: Returns list of SSH commands to run
- `execute_commands(commands)`: Runs commands, returns (outputs, error)
- `process_outputs(outputs, ip, self.outputs)`: Parses and stores data
- `stop_callback`: Passed to halt execution cooperatively
- `abort_connection()`: Forcefully closes SSH/Telnet connections
